...
export class GlobalArbiter {
  static planHouse(
    ...
    for (const wallId of priorityIds) {
      ...
      if (topLocal.length === 0) {
          throw new Error(`CRITICAL_PLANNING_FAILURE: No valid local candidates provided for wall ${wallId}.`);
      }
      ...
      // 4 & 5. Validate & Hard Veto
      for (const partialPlan of nextBeam) {
          const validation = validateGlobalPlan(partialPlan.wallSelections, house.walls);
          if (!validation.valid) {
          ...
          // 6. Score valid plans
          partialPlan.partialScore = scorePartialGlobalPlan(partialPlan, house.walls);
          validNextBeam.push(partialPlan);
      }

      // 7 & 8. Prune weak plans & Collapse equivalents
      const prunedBeam = this.pruneBeam(validNextBeam, config, collector, house.walls);
    ...
    const finalCandidates: GlobalPlanCandidate[] = beam.map(p => {
        ...
        candidate.score = scoreFinalGlobalPlan(candidate, house.walls);
        return candidate;
    });

    // 12. Select winner (Tie-break by lexical ID as fallback)
  ...
  private static pruneBeam(
    ...
    // Equivalency Collapse (Duplicate-equivalent partial branches collapse)
